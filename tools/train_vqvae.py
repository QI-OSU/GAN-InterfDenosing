...
def train(args):
    ...
    if train_config['distributed']:
        ...
        lpips_model = torch.nn.parallel.DistributedDataParallel(lpips_model, device_ids=[0, 1, 2, 3])
        lpips_model_without_ddp = lpips_model.module

        discriminator = torch.nn.parallel.DistributedDataParallel(discriminator, device_ids=[0, 1, 2, 3])
    ...
    step_count, resume_epoch = 0, 0

    # This is for accumulating gradients incase the images are huge
    # And one cant afford higher batch sizes
    acc_steps = train_config['autoencoder_acc_steps']
    (epoch_lst, recon_loss_epoch, perceptual_loss_epoch, phs_loss_epoch,
    ...
    if train_config['resume']:
        ...
        recon_loss_epoch = recon_loss_dict['loss'][:resume_epoch]
        perceptual_loss_epoch = perceptual_loss_dict['loss'][:resume_epoch]
        phs_loss_epoch = phs_loss_dict['loss'][:resume_epoch]
        gen_loss_epoch = gen_loss_dict['loss'][:resume_epoch]
        disc_loss_epoch = disc_loss_dict['loss'][:resume_epoch]
        epoch_lst = recon_loss_dict['epoch'][:resume_epoch]

        print(f'---resume from epoch: {resume_epoch}---')

    for epoch_idx in range(resume_epoch, resume_epoch + num_epochs):
        loss_dict = train_one_epoch(model, lpips_model, discriminator, data_loader,
        ...
        if loss_dict.get('gen_loss'):
            gen_loss_epoch.append(loss_dict['gen_loss'].item())
            loss_lst.append('gen_loss_epoch')
        if loss_dict.get('disc_loss'):
            disc_loss_epoch.append(loss_dict['disc_loss'].item())
            loss_lst.append('disc_loss_epoch')
